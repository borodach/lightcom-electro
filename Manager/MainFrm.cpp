...
{
    m_wndStatusBar.SetPaneText (0, L"Загрузка списка потребителей из файла...");
    ...
    if (! bResult)
    {
        m_wndStatusBar.SetPaneText (0, L"Не удалось загрузить список потребителей.");
    }
    else
    {
        m_wndStatusBar.SetPaneText (0, L"Список потребителей успешно загружен.");
    }
...
{
    m_wndStatusBar.SetPaneText (0, L"Сохранение списка потребителей в файл...");
    ...
    {
        SetModified (false);
        m_wndStatusBar.SetPaneText (0, L"Список потребителей успешно сохранён.");

        return true;
    }
    else
    {
        m_wndStatusBar.SetPaneText (0, L"Не удалось сохранить список потребителей.");
...
void CMainFrame::UpdateHtmlView (bool bReload, bool bRenderXML)
{
    ...
    try
    {
        ...
        {
            m_wndStatusBar.SetPaneText (0, L"Загрузка результатов сканирования счетчиков...");
            m_DataMgr.SetCurrentCustomerIdx (nCustomerIdx, strBaseFolder + DEFAULT_RESULTS_FOLDER);
            m_wndStatusBar.SetPaneText (0, L"Загрузка результатов сканирования счетчиков завершена");
        }

        if (nCurrentCustomerIdx != nCustomerIdx) bRenderXML = true;

        //
        // Генерируем XML отчеты
        //
    ...
    {
        MessageBox (L"При создании отчета возникла неожиданная ошибка.", L"Ошибка", MB_ICONERROR);
    }
}
...
void CMainFrame::GenerateXMLFiles ()
{
...

    {
        ...
        if (NULL != xmlDocument)
        {
            m_wndStatusBar.SetPaneText (0, L"Генерирование общего отчёта...");
            MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateSummaryReport (xmlDocument); 
            if (NULL != ptrElement)
            {
                xmlDocument->documentElement->appendChild (ptrElement);
                m_wndStatusBar.SetPaneText (0, L"Общий отчёт успешно сгенерирован");
                m_wndStatusBar.SetPaneText (0, L"Сохранение общего отчёта...");
                _variant_t fileName (strXMLFile);
                HRESULT hr = xmlDocument->save (fileName);
                if (FAILED (hr)) 
                {
                    m_wndStatusBar.SetPaneText (0, L"Не удалось сохранить общий отчёт");
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, L"Общий отчёт успешно сохранён");
                }
            }
            else
            {
                m_wndStatusBar.SetPaneText (0, L"Не удалось сгенерировать общий отчёт");
            }
        }
        m_wndStatusBar.SetPaneText (0, L"Общий отчёт создан");
    }
...

    {
        ...
        if (NULL != xmlDocument)
        {
            m_wndStatusBar.SetPaneText (0, L"Генерирование отчёта по потребителю...");
            MSXML::IXMLDOMElementPtr ptrElement = 
            ...
            {
                xmlDocument->documentElement->appendChild (ptrElement);
                m_wndStatusBar.SetPaneText (0, L"Отчёт по потребителю успешно сгенерирован");
                m_wndStatusBar.SetPaneText (0, L"Сохранение отчёта по потребителю...");
                _variant_t fileName (strXMLFile);
                HRESULT hr = xmlDocument->save (fileName);
                if (FAILED (hr)) 
                {
                    m_wndStatusBar.SetPaneText (0, L"Не удалось сохранить отчёт по потребителю");
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, L"Отчёт по потребителю успешно сохранён");
                }
            }
            else
            {
                m_wndStatusBar.SetPaneText (0, L"Не удалось сгенерировать отчёт по потребителю");
            }
        }
        m_wndStatusBar.SetPaneText (0, L"Отчёт по потребителю создан");
    }

    //
    // Суммарный отчёт по энергопотреблению
    //

    {
        ...
        if (NULL != xmlDocument)
        {
            m_wndStatusBar.SetPaneText (0, L"Генерирование отчёта по суммарному энергопотреблению...");
            MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCustomerEnergyReport (xmlDocument); 
            if (NULL != ptrElement)
            {
                xmlDocument->documentElement->appendChild (ptrElement);
                m_wndStatusBar.SetPaneText (0, L"Отчёт по суммарному энергопотреблению успешно сгенерирован");
                m_wndStatusBar.SetPaneText (0, L"Сохранение отчёта по суммарному энергопотреблению...");
                _variant_t fileName (strXMLFile);
                HRESULT hr = xmlDocument->save (fileName);
                if (FAILED (hr)) 
                {
                    m_wndStatusBar.SetPaneText (0, L"Не удалось сохранить отчёт по суммарному энергопотреблению");
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, L"Отчёт по суммарному энергопотреблению успешно сохранён");
                }
            }
            else
            {
                m_wndStatusBar.SetPaneText (0, L"Не удалось сгенерировать отчёт по суммарному энергопотреблению");
            }
        }
        m_wndStatusBar.SetPaneText (0, L"Отчёт по суммарному энергопотреблению создан");
    }

    //
    // Суммарный отчёт по мощности
    //
    ...
    PowerUsageSummarizer summaryPowerReport;
    {
        ...
        if (NULL != xmlDocument)
        {
            m_wndStatusBar.SetPaneText (0, L"Генерирование отчёта по суммарному потреблению мощности...");
            MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCustomerSummaryPowerReport (xmlDocument, summaryPowerReport); 
            if (NULL != ptrElement)
            {
                xmlDocument->documentElement->appendChild (ptrElement);
                m_wndStatusBar.SetPaneText (0, L"Отчёт по суммарному потреблению мощности успешно сгенерирован");
                m_wndStatusBar.SetPaneText (0, L"Сохранение отчёта по суммарному потреблению мощности...");
                _variant_t fileName (strXMLFile);
                HRESULT hr = xmlDocument->save (fileName);
                if (FAILED (hr)) 
                {
                    m_wndStatusBar.SetPaneText (0, L"Не удалось сохранить отчёт по суммарному потреблению мощности");
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, L"Отчёт по суммарному потреблению мощности успешно сохранён");
                }
            }
            else
            {
                m_wndStatusBar.SetPaneText (0, L"Не удалось сгенерировать отчёт по суммарному потреблению мощности");
            }
        }
        m_wndStatusBar.SetPaneText (0, L"Отчёт по суммарному потреблению мощности создан");
    }
    
    //
    // Помесячный отчёт по мощности
    //

    {
        ...
        if (NULL != xmlDocument)
        {
            m_wndStatusBar.SetPaneText (0, L"Генерирование отчёта по помесячному потреблению мощности...");
            MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCustomerMonthlyPowerReport (xmlDocument, 
                                                                                                summaryPowerReport.GetStorage ()); 
            if (NULL != ptrElement)
            {
                xmlDocument->documentElement->appendChild (ptrElement);
                m_wndStatusBar.SetPaneText (0, L"Отчёт по помесячному потреблению мощности успешно сгенерирован");
                m_wndStatusBar.SetPaneText (0, L"Сохранение отчёта по помесячному потреблению мощности...");
                _variant_t fileName (strXMLFile);
                HRESULT hr = xmlDocument->save (fileName);
                if (FAILED (hr)) 
                {
                    m_wndStatusBar.SetPaneText (0, L"Не удалось сохранить отчёт по помесячному потреблению мощности");
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, L"Отчёт по помесячному потреблению мощности успешно сохранён");
                }
            }
            else
            {
                m_wndStatusBar.SetPaneText (0, L"Не удалось сгенерировать отчёт по помесячному потреблению мощности");
            }
        }
        m_wndStatusBar.SetPaneText (0, L"Отчёт по помесячному потреблению мощности");
    }
    ...
    m_wndStatusBar.SetPaneText (0, L"Отчёты по счётчикам...");
    ...
    for (size_t nIdx = 0; nIdx < nCounterCount; ++ nIdx)
    {
...

        {
            ...
            if (NULL != xmlDocument)
            {
                m_wndStatusBar.SetPaneText (0, strPrefix + L"Генерирование общего отчёта...");
                MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCounterReport (xmlDocument, nIdx);
                if (NULL != ptrElement)
                {
                    xmlDocument->documentElement->appendChild (ptrElement);
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Общий отчёт успешно сгенерирован");
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Сохранение общего отчёта...");
                    _variant_t fileName (strXMLFile);             
                    HRESULT hr = xmlDocument->save (fileName);
                    if (FAILED (hr)) 
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сохранить общий отчёт");
                    }
                    else
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Общий отчёт успешно сохранён");
                    }
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сгенерировать общий отчёт");
                }
            }
        }
...

        {
            ...
            if (NULL != xmlDocument)
            {
                m_wndStatusBar.SetPaneText (0, strPrefix + L"Генерирование журнала событий...");
                MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCounterEventLog (xmlDocument, nIdx);
                if (NULL != ptrElement)
                {
                    xmlDocument->documentElement->appendChild (ptrElement);
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Журнал событий успешно сгенерирован");
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Сохранение журнала событий...");
                    _variant_t fileName (strXMLFile);
                    HRESULT hr = xmlDocument->save (fileName);
                    if (FAILED (hr)) 
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сохранить журнал событий");
                    }
                    else
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Журнал событий успешно сохранён");
                    }
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сгенерировать журнал событий");
                }
            }
        }
...

        {
            ...
            if (NULL != xmlDocument)
            {
                m_wndStatusBar.SetPaneText (0, strPrefix + L"Генерирование отчёта по энергопотреблению...");
                MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCounterEnergyReport (xmlDocument, nIdx);
                if (NULL != ptrElement)
                {
                    xmlDocument->documentElement->appendChild (ptrElement);
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Отчёт по энергопотреблению успешно сгенерирован");
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Сохранение отчёта по энергопотреблению...");
                    _variant_t fileName (strXMLFile);
                    HRESULT hr = xmlDocument->save (fileName);
                    if (FAILED (hr)) 
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сохранить отчёт по энергопотреблению");
                    }
                    else
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Отчёт по энергопотреблению успешно сохранён");
                    }
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сгенерировать отчёт по энергопотреблению");
                }
            }
        }
...

        {
            ...
            if (NULL != xmlDocument)
            {
                m_wndStatusBar.SetPaneText (0, strPrefix + L"Генерирование отчёта по мощности...");
                MSXML::IXMLDOMElementPtr ptrElement = m_DataMgr.GenerateCounterPowerReport (xmlDocument, nIdx);
                if (NULL != ptrElement)
                {
                    xmlDocument->documentElement->appendChild (ptrElement);
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Отчёт по мощности успешно сгенерирован");
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Сохранение отчёта по мощности...");
                    _variant_t fileName (strXMLFile);
                    HRESULT hr = xmlDocument->save (fileName);
                    if (FAILED (hr)) 
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сохранить отчёт по мощности");
                    }
                    else
                    {
                        m_wndStatusBar.SetPaneText (0, strPrefix + L"Отчёт по мощности успешно сохранён");
                    }
                }
                else
                {
                    m_wndStatusBar.SetPaneText (0, strPrefix + L"Не удалось сгенерировать отчёт по мощности");
                }
            }
        }
    }
